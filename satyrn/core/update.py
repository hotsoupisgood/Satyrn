import satyrn.core.database as Database
...
def checkUpdate(socketio, fileLocation):
    #Get file target information from database if it exists
    HashList, Cells, GlobalScope, LocalScope  = Database.getLedger(fileLocation)
    #If it's modified or if it's the first time it has run, run update
    if isModified(fileLocation) or not GlobalScope:
        time.sleep(1)
        update(socketio, fileLocation, GlobalScope, LocalScope, HashList, Cells)
        time.sleep(1)
    else:
        time.sleep(1)
...
def update(socketio, fileLocation, GlobalScope, LocalScope, HashList, Cells):
    ...
    hashList, Cells=Ledger.updateLedgerPop(Cells, fileContent, HashList, myDir)
    ...
    '''
    output=Run.runNewCells(Cells, hashList, GlobalScope, LocalScope, myDir)
    ...
    Database.updateLedger(fileLocation, hashList, Cells, GlobalScope, LocalScope, executions)
